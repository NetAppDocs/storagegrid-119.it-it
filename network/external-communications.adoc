...
*Nota*: se chiudi le porte 443 o 8443 di Grid Manager, tutti gli utenti attualmente connessi su una porta bloccata, incluso te, perderanno l'accesso a Grid Manager, a meno che il loro indirizzo IP non sia stato aggiunto all'elenco degli indirizzi privilegiati.  Vederelink:../admin/configure-firewall-controls.html["Configurare i controlli del firewall"] per configurare indirizzi IP privilegiati.
